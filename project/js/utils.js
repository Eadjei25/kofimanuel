export const APIKey = "5c78240201b345f0bb87ad182cdeda5e";
export const recipesDiv = document.getElementById("recipes-div");

export async function search(string) {
	try {
		let response = await fetch(
			`https://api.spoonacular.com/recipes/complexSearch?apiKey=${APIKey}&query=${string}&addRecipeInformation=true&addRecipeNutrition=true`
		);
		if (!response.ok) {
			...
			if (response.status == 402) {
				error.textContent =
					"Sorry, all possible searches have been made today. Try Again tomorrow!";
